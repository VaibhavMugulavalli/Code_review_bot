...
"""You are a code reviewer. Compare the user's solution with the optimized one, print both user soultion and optimized solution and only compare the time and space complexity between the two approaches.
User Solution:
{user_solution}
...
def parse_explanation(response):
    detailed_explanation=response.content.strip()
    print(detailed_explanation)
    return{"detailed_explanation":detailed_explanation}

explanation_chain=explanation_prompt | genai | RunnableLambda(parse_explanation)


def human_in_loop(state: CodeReviewState) -> dict:
    user_response = input("Are you satisfied with the feedback? (Yes/No): ").strip().lower()
    return {"user_satisfied": user_response == "yes"}
...
if __name__ == "__main__":
    ...
    solution=click.edit("Enter your solution\n")
    initial_state ={"question":question,"user_solution":solution}
